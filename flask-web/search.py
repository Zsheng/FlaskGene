...
class HandleInfo:
    def __init__(self, chr, start, end, variant_type, table):
        ...
        if table == 'book':
            self.columns = '`ID`,`Syndrome`,`OMIM`,`Chromo`,`Location`,`Start`,`End`,' \
            ...
            self.columns = "`ID`, `Chromo`, `Start`, `End`, `Variant_Call_type`, " \
            ...
            self.columns = "`ID`, `Chromo`, `Start`, `End`, `variantsubtype`, " \
                           "`pubmedid`, `observedgains`, `observedlosses`, `genes`"
            self.columns_type = ''
        elif table == 'GenemapOmim':
            self.columns = "`Chromo`, `Start`, `End`, `Gene_Symbol`, `MIM_Number`, " \
                      "`Disorders`, `Cytogenetic_Location`, `Genomic_Coordinates`, `Entrez_Gene_ID`"
            self.columns_type = ''
        else:
    ...
    def handle_sql(self):
        sql = "select %s from %s WHERE `Chromo`= '%s' AND %s " \
              "cast(`Start` AS unsigned INT )<=%s and  cast(`End` as unsigned INT )>=%s" \
              % (self.columns, self.table, self.chr, self.columns_type, self.start, self.end)
        # print sql
        return sql
...
def write_data(columns, datas, file_name, flag):
    name = time.strftime("%y%m%d%H%M", time.localtime(time.time()))+file_name+flag+'.csv'
